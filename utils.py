...
def create_dataset(dataset=None, read_part=True, sample_num=100000, task='classification', sequence_length=40, device=torch.device('cpu')):
    if dataset == 'criteo':
        return CriteoDataset('/home/wan/Desktop/Brucewan/Sequence_rec_baseline/DIEN_pytorch/dataset/criteo-100k.txt', read_part=read_part, sample_num=sample_num).to(device)
    elif dataset == 'movielens':
        return MovieLensDataset('/home/wan/Desktop/Brucewan/Sequence_rec_baseline/DIEN_pytorch/dataset/ml-latest-small-ratings.txt', read_part=read_part, sample_num=sample_num, task=task).to(device)
    elif dataset == 'amazon-books':
        return AmazonBooksDataset('/home/wan/Desktop/Brucewan/Sequence_rec_baseline/DIEN_pytorch/dataset/amazon-books-100k.txt', read_part=read_part, sample_num=sample_num, sequence_length=sequence_length).to(device)
    else:
...
class Trainer:
    ...
    def test(self, test_X, test_y):
        ...
        if self.task == 'classification':
            test_metric = metrics.roc_auc_score(test_y.cpu(), pred_y.cpu()) # 分类问题评判用roc
